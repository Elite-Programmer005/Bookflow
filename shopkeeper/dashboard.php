...
<div class="grid gap-8 lg:grid-cols-[260px_1fr]">
    <aside class="rounded-[2rem] bg-slate-900 p-6 text-white shadow-2xl shadow-slate-300/40">
        ...
        <nav class="mt-8 space-y-2 text-sm font-medium">
            ...
            <a href="<?= e(app_url('/shopkeeper/settings')) ?>" class="block rounded-2xl px-4 py-3 hover:bg-white/10">Settings</a>
        </nav>
    </aside>
    ...
    <section class="space-y-8">
        ...
        <div class="rounded-[2rem] bg-white p-6 shadow-xl">
            <div class="flex items-center justify-between gap-4">
                ...
                <a href="<?= e(app_url('/shopkeeper/bookings')) ?>" class="rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold text-slate-700">Open bookings</a>
            </div>
            <div class="mt-6 space-y-4">
                <?php foreach (array_slice($bookings, 0, 8) as $booking): ?>
                    <div class="flex flex-col gap-3 rounded-3xl border border-slate-100 p-5 md:flex-row md:items-center md:justify-between">
                        <div>
